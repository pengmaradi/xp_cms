...
class MPTtree_ORM_node{
     ...
     */
    /* 
	public function MPTtree_ORM_node(&$instance,$data = false){
		$this->init($instance, $data);
	}
    */
    
	public function __construct(&$instance,$data = false) {
        $this->init($instance, $data);
    }
	/**
	 * Initiates the node with MPTtree instance and lft value.
	 ...
	 */
	public function init(&$instance,&$data){
	 ...
	 */
	public function refresh($children = false){
		if($this->orphan)
			return false;
	 ...
	 */
	public function path($true_path = false){
		// if this is an orphan, return false
		if($this->orphan)
				return false;
	 ...
	 */
	public function xpath($path, $separator = '/',$relative = true){
		if($relative)
	 ...
	 */
	public function insert_above(&$node){
	 ...
	 * @todo Update the parent node, so it's children gets updated
	 */
	public function insert_below(&$node){
		if($node->lft == 1 || !$this->orphan  || $node->orphan || $node->instance->tree_table != $this->instance->tree_table)
			return false;
	 ...
	 * @todo Update the parent node, so it's children gets updated
	 */
	public function move_above(&$node){
		if($this->orphan)
	 ...
	 * @todo Update the parent node, so it's children gets updated
	 */
	public function move_below(&$node){
		if($this->orphan)
	 ...
	 * @return false if insert failed, true if success
	 */	
	public function move_to_first_child_of(&$node){
		if($this->orphan)
	 ...
	 * @return false if insert failed, true if success
	 */
	public function move_to_last_child_of(&$node){
		if($this->orphan)
