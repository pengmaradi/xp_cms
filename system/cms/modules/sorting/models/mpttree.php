...
class MPTtree extends CI_Model {
     ...
     */
    private static $inc_dir = '/models/MPTtree';

    /**
     * Variable to determine if ORM file has been loaded.
     */
    private static $ORM_enabled = false;

    /**
     * Variable to determine if iterator file has been loaded.
     */
    private static $iterator_enabled = false;
     ...
     * @arg <b>uid</b> The name of the column which holds the unique id, default: 'uid'
    ...
    public function set_opts($opts) {
        ...
        $uid = isset($opts['uid']) ? $opts['uid'] : 'uid';
     ...
     */
    public function enable_iterator() {
     ...
     */
    public function iterator($lft = 0) {
        if (!$this->enable_iterator())
            return false;
     ...
     */
    public function ORMiterator($lft = 0) {
        if (!$this->enable_iterator())
            return false;
    ...
    public function get_ORM_byid($id = 1) {
        $this->enable_ORM();
     ...
     * @since 0.1.4
     */
    public function lock_tree_table($aliases = array()) {
     ...
     * @since 0.1.4
     */
    public function unlock_tree_table() {
        $q = "UNLOCK TABLES";
        $res = $this->db->query($q);
    }
     ...
     * but if no rows returned, false
     */
    public function get_node_byid($id) {
     ...
     */
    public function get_descendants($lft, $rgt, $with_level_col = false) {
        if ($with_level_col)
     ...
     * but if no rows returned, empty array
     */
    public function get_descendants_wlevel($lft, $rgt) {
...
ORDER BY node.{$this->left_col};");
     ...
     * but if no rows returned, empty array
     */
    public function get_descendants_where($lft, $rgt, $where) {
        $this->db->where($this->left_col . ' >', $lft);
        $this->db->where($this->right_col . ' <', $rgt);
        $this->db->where($where);
        $this->db->order_by($this->left_col, 'asc');
     ...
     */
    public function count_descendants($lft, $rgt) {
        return (($rgt - $lft) - 1) / 2;
    }
     ...
     * but if no rows returned, empty array
     */
    public function get_children($lft, $rgt) {
        if ($rgt - $lft < 3) // leaf node, 3 here because of the possibility of a gap (4 = have children)
            return array();
...
ORDER BY node.{$this->left_col};");
     ...
     * but if no rows returned, empty array
     */
    public function get_children_where($lft, $rgt, $where) {
        if ($rgt - $lft < 3) // leaf node, 3 here because of the possibility of a gap (4 = have children)
            return array();
        $this->db->select('*');
...
ORDER BY node.{$this->left_col}) as a";
     ...
     */
    public function count_children($lft, $rgt) {
        $result = $this->db->query(
...
ORDER BY node.{$this->left_col}) as a");
     ...
     * but if no rows returned, empty array
     */
    public function get_parents($lft, $rgt) {
        $this->db->where($this->left_col . ' <', $lft);
        $this->db->where($this->right_col . ' >', $rgt);
     ...
     * but if no rows returned, false
     */
    public function get_parent($lft, $rgt) {
        $this->db->where($this->left_col . ' <', $lft);
        $this->db->where($this->right_col . ' >', $rgt);
        $this->db->order_by($this->left_col, 'desc');
     ...
     */
    public function xpath($path, $separator = '/', $root = 1) {
        if (is_array($path)) {
            $segments = $path;
        } else {
            // split the segments
     ...
     * @return An array with the requested node's data
     */
    public function xpath2($path, $separator = '/', $root = 1) {
        if (is_array($path)) {
            $segments = $path;
        } else {
            // split the segments
...
ORDER BY node.{$this->left_col}) as a";
     ...
     */
    public function fetch_path($path, $separator = '/') {
     ...
     */
    public function tree2array($root = 1) {
...
//echo $this->db->last_query();
    ...
    /**
     * Puts the get_descendts() query in an Actve Record subquery, so you can use sorting, custom filtering etc.
     ...
     * @return void
     */
    public function AR_from_descendants_of($lft, $rgt) {
        // Circumvent the db escaping to enable a subquery
...
ORDER BY {$this->left_col} ASC) as descendant";
     ...
     * @return void
     */
    public function AR_from_children_of($lft, $rgt) {
        // Circumvent the db escaping to enable a subquery
...
ORDER BY node.{$this->left_col}) as child";
     ...
     * @return void
     */
    public function AR_from_parents_of($lft, $rgt) {
        // Circumvent the db escaping to enable a subquery
        $this->db->ar_from[] = "(SELECT * FROM {$this->tree_table}
WHERE {$this->left_col} < $lft AND {$this->right_col} > $rgt
ORDER BY {$this->left_col} DESC) as parent";
     ...
     */
    public function update_node($lft, $data) {
     ...
     */
    public function insert_root($data) {
     ...
     */
    public function insert_node_before($lft, $data) {
        if (!$this->get_node($lft))
            return false;
     ...
     * @return array with the new lft, rgt and id values, False otherwise
     */
    public function insert_node_after($lft, $data) {
     ...
     * @return array with the new lft, rgt and id values, False otherwise
     */
    public function append_node($lft, $data) {
        if (!$this->get_node($lft))
            return false;
     ...
     * @return array with the new lft, rgt and id values, False otherwise
     */
    public function append_node_last($lft, $data) {
        $node = $this->get_node($lft);
        if (!$node)
            return false;
     ...
     * @return array with the new lft, rgt and id values, False otherwise
     */
    public function insert_sorted($lft, $data) {
     ...
     * @return array with the new lft, rgt and id values, False otherwise
     */
    public function insert_node($lft, $data, $lock = true) {
        $root = $this->get_root();
        if ($lft > $root[$this->right_col] || $lft < 1)
            return false;
        $data = $this->sanitize_data($data);
        ...
        // HCG: I have deprecated the use of the public function create_space here - I regard it as good practice NOT
        // to have public function calls when a table is locked. NOTE: Under mySQL, you can only lock a table ONCE from
        // a single process.
        ...
        // fetch the id
        $this->db->select($this->id_col);
        $data = $this->db->get_where($this->tree_table, array($this->left_col => $lft));
        $ret = $data->row_array();

        if ($lock)
            $this->unlock_tree_table();
        return array($lft, $lft + 1, $ret[$this->id_col]);
    }
     ...
     */
    public function move_node_before($lft, $nlft) {
     ...
     * @since 0.1
     * @param $lft The lft of the node to be moved
     * @param $nlft The lft of the node which it will be place after
     * @return array with the new lft and rgt values if node moved, false if not
     */
    public function move_node_after($lft, $nlft) {
        $node = $this->get_node($nlft);
        if (!$node)
            return false;
     ...
     * @return array with the new lft and rgt values if node moved, false if not
     */
    public function move_node_append($lft, $nlft) {
        $node = $this->get_node($nlft);
        if (!$node)
            return false;
        return $this->move_node($lft, $nlft + 1);
     ...
     * @return array with the new lft and rgt values if node moved, false if not
     */
    public function move_node_append_last($lft, $nlft) {
        $node = $this->get_node($nlft);
        if (!$node)
            return false;
     ...
     * @return array with the new lft and rgt values if node moved, false if not
     */
    public function move_node($lft, $nlft) {
        // Validate values
        $node = $this->get_node($lft);
        if (!$node || $lft == $nlft)
            return false;
        $root = $this->get_root();
        if ($nlft > $root[$this->right_col] || $nlft < 2 || $lft == 1)
            return false;
        ...
        ///korr tz
        $node = $this->get_node_byid($node[$this->id_col]);
        $this->unlock_tree_table();
        return $node;
    }
     ...
     */
    public function delete_node($lft) {
        $node = $this->get_node($lft);
     ...
     * @return True if something was deleted, false if not
     */
    public function delete_branch($lft) {
        $node = $this->get_node($lft);
        if (!$node || $node[$this->left_col] == 1)
            return false;
        // lock table
    ...
    //  Gap functions
    //////////////////////////////////////////////
    // HCG: I do not like ANY of these functions because I HATE having public function calls involved in scenarios where table locking is done.
    ...
    //
	// All calls to the Gap functions below MUST be called from within a public function which has acquired a table lock, otherwise IT WILL BREAK!
     ...
     */
    public function create_space($pos, $size) {
        $root = $this->get_root();
        if ($pos > $root[$this->right_col] || $pos < $root[$this->left_col])
            return false;
        $this->db->query('UPDATE ' . $this->tree_table .
     ...
     */
    public function remove_first_gap() {
     ...
     */
    public function is_valid_node($lft) {
        $node = $this->get_node($lft);
        if (!$node)
            return false;
     ...
     */
    public function validate($ret = true) {
        $this->db->select($this->left_col . ', ' . $this->right_col);
        $query = $this->db->get($this->tree_table);
        $lftrgt = array();
        $lfts = array();
     ...
     */
    /* private function _protect_identifiers($ident)
      {
