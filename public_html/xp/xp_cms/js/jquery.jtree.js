 ...
 * Requires: jQuery 1.2+
 *
 * Anpassungen an MPTree/Codeigniter
 * webprogrammer@eb-zuerich.ch juni 2010
 *
 * click auf li-element wird nicht weitergeleitet
 *
 * click auf a wird nicht als drag-unddrop-versuch gewertet
 *
 * click auf button name="del" wird mit eigener function deleteIt verknüpft, falls selbige vorhanden
 *
 * mouseup nach drag&drop wird mit eigener function mouseUp verknüpft, falls selbige vorhanden
 * 
 * deleteIt wie fetchIt werden den options mitgegeben => tree_view.php
 *	...
 *	mouseUp:fetchIt,
 *	deleteIt:deleteItem
*/
	...
	$.fn.jTree = function(options) {
		...
		$(this).find("li").mousedown(function(e){
			...
			if(e.target.name=='del') {
				if(jQuery.isFunction(opts.deleteIt)) {
					opts.deleteIt(e.target);
					return false;
				}
			}
			/// end tz
		...
		$(this).find("li").andSelf().mouseup(function(e){
			...
			///tz
			if(jQuery.isFunction(opts.mouseUp)) {
				opts.mouseUp(cur);
			}
			///end tz
